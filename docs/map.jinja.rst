...
We will see a quick setup to configure the ``tool_yabai`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_yabai`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_yabai`` formulas under ``/srv/salt/tool_yabai/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_yabai/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_yabai/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_yabai/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_yabai/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_yabai`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_yabai`` formulas under ``/srv/salt/tool_yabai/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_yabai/parameters/roles
...
We create a configuration for the role ``tool_yabai/server`` in ``/srv/salt/tool_yabai/parameters/roles/tool_yabai/server.yaml``:
...
We create another configuration for the role ``tool_yabai/client`` in ``/srv/salt/tool_yabai/parameters/roles/tool_yabai/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_yabai/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_yabai/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_yabai/parameters/dns:domain/  '/srv/salt/tool_yabai/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-yabai-as-json:
      test.nop:
        - name: {{ yabai | json }}
...
.. code-block:: sls
    ...
    yabai-config-file-file-managed:
      file.managed:
        - name: {{ yabai.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='yabai-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ yabai.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``yabai`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ yabai['winner'] }}
